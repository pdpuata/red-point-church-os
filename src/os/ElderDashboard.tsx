...
export default function ElderDashboard({onAction}:Props){
 ...
 return <View style={{padding:20,paddingBottom:24}}>
  ...
  <View style={card}>
   ...
   {onAction?<Pressable accessibilityRole="button" accessibilityLabel="Review Sunday readiness" onPress={()=>onAction('sunday')} style={{marginTop:14,borderWidth:1,borderColor:'#171717',borderRadius:12,padding:13,alignItems:'center'}}><Text style={{fontWeight:'800'}}>REVIEW SUNDAY</Text></Pressable>:null}
  </View>
  {ready?<View style={{...card,backgroundColor:'#f1f1ed'}}><Text style={{fontSize:17,fontWeight:'800'}}>✓ Everything is under control</Text><Text style={{fontSize:15,lineHeight:22,color:'#555',marginTop:5}}>There are no current decisions waiting for you.</Text></View>:<View style={card}><Text style={{fontSize:19,fontWeight:'800'}}>What needs my attention?</Text>{attention.map((item:any)=><Pressable key={item.key} accessibilityRole={onAction?'button':undefined} disabled={!onAction} onPress={()=>{if(item.key==='visitor_followup')onAction?.('visitors');else if(item.key==='roster_confirmation'||item.key==='roster_missing'||item.key==='setlist_missing')onAction?.('music');else onAction?.('sunday');}} style={{borderTopWidth:1,borderTopColor:'#e8e8e5',paddingVertical:14,marginTop:10}}><Text style={{fontSize:11,fontWeight:'800',letterSpacing:1,color:item.severity==='high'?'#B42318':'#666'}}>{item.severity==='high'?'NEEDS ATTENTION':'REVIEW'}</Text><Text style={{fontSize:17,fontWeight:'800',color:'#171717',marginTop:4}}>{item.title}</Text><Text style={{fontSize:14,lineHeight:20,color:'#555',marginTop:3}}>{item.detail}</Text>{onAction?<Text style={{fontSize:13,fontWeight:'800',marginTop:7}}>REVIEW ›</Text>:null}</Pressable>)}</View>}
  <View style={card}><Text style={{fontSize:19,fontWeight:'800'}}>Church areas</Text><Text style={{fontSize:14,lineHeight:21,color:'#666',marginTop:4,marginBottom:8}}>Open an area only when you need to make a decision.</Text>{[['MUSIC & BANDS','People, bands, roster and worship readiness','music'],['PEOPLE','Visitors and people who need a response','visitors'],['SUNDAY','The simple pre-Sunday checklist','sunday']].map(([title,body,action])=>onAction?<Pressable key={action} onPress={()=>onAction(action as ElderDashboardAction)} style={{borderTopWidth:1,borderTopColor:'#e8e8e5',paddingVertical:13}}><Text style={{fontSize:16,fontWeight:'800'}}>{title}</Text><Text style={{fontSize:14,color:'#666',marginTop:3}}>{body}</Text></Pressable>:<View key={action} style={{borderTopWidth:1,borderTopColor:'#e8e8e5',paddingVertical:13}}><Text style={{fontSize:16,fontWeight:'800'}}>{title}</Text><Text style={{fontSize:14,color:'#666',marginTop:3}}>{body}</Text></View>)}</View>
