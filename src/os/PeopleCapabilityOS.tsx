...
export default function PeopleCapabilityOS() {
  ...
  return <View>
    <View style={card}>
      ...
      <Text style={{color:'#666',marginTop:4}}>This is the organisational memory behind roster intelligence. The system must know who people are, what they can do, where they belong, and how they are developing.</Text>
    ...
    {selected ? <View style={card}>
      ...
      <Text style={{fontWeight:'800',marginTop:14}}>Current operational graph</Text>
      ...
      <Text style={{fontWeight:'800',marginTop:18}}>Add to band</Text>
    ...
    <View style={card}>
      <Text style={{fontSize:17,fontWeight:'800'}}>Workload & Fairness Intelligence</Text>
      <Text style={{color:'#555',lineHeight:21,marginTop:6}}>The roster engine now treats workload as an operational signal: recent service load, recency, availability and capability can inform recommendations without allowing AI to make the final people decision.</Text>
      {workload.length === 0 ? <Text style={{color:'#777',marginTop:10}}>No service history exists yet, so fairness intelligence has no evidence to calculate.</Text> : workload.slice(0,12).map((w:any)=><View key={w.user_id} style={{marginTop:10,padding:11,borderWidth:1,borderColor:'#e3e3e0',borderRadius:12}}>
        <View style={{flexDirection:'row',justifyContent:'space-between'}}><Text style={{fontWeight:'800',flex:1}}>{w.display_name}</Text><Text style={{fontWeight:'800'}}>{w.workload_band}</Text></View>
        <Text style={{color:'#666',marginTop:4}}>8 weeks: {w.services_8w} · 4 weeks: {w.services_4w} · 2 weeks: {w.services_2w}</Text>
        <Text style={{color:'#666',marginTop:3}}>Last served: {w.last_service_date || 'never'} · Bands: {w.band_count}</Text>
      </View>)}
    </View>

    <View style={card}><Text style={{fontSize:17,fontWeight:'800'}}>Why this matters</Text><Text style={{color:'#555',lineHeight:21,marginTop:6}}>Once this graph is populated, roster planning can move from “no candidates” to evidence-backed recommendations. It also becomes the foundation for training, succession, scheduling, fairness, and workload balancing.</Text></View>
  </View>;
}
