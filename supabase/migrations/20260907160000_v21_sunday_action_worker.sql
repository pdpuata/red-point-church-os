-- v21: make the Sunday loop act safely.
-- Autonomous actions are limited to preparing routine confirmation communications and
-- escalating declined assignments. Staffing changes and external sends remain human/credential gated.
...
begin
  for s in
    select id, service_date, band_id
    ...
    where service_date between current_date and current_date + public.os_runtime_policy_int('runtime.service_horizon_days', 7)
      and service_type = 'sunday'
      and status <> 'cancelled'
    order by service_date
    limit public.os_runtime_policy_int('runtime.max_services', 25)
  loop
    begin
      service_result := public.os_run_sunday_operating_loop_internal(s.id);
      for a in
      ...
      loop
        ...
        msg_body := format(
          'Hi %s, you are assigned to serve as %s on %s. Please confirm that you are available.',
  ...
  return jsonb_build_object(
    ...
    'services_processed', (select count(*) from public.music_services where service_date between current_date and current_date + public.os_runtime_policy_int('runtime.service_horizon_days', 7) and service_type='sunday_service' and status <> 'cancelled'),
    'communications_prepared', prepared,
    'unlinked_recipients_skipped', skipped_unlinked,
    'replacement_approvals_escalated', replacement_escalations,
    'errors', errors,
    'ran_at', now()
  );
end;
...
select cron.schedule('sunday-action-worker', '*/5 * * * *', 'select public.os_run_sunday_action_worker();');
