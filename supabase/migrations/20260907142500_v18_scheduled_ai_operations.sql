-- Scheduled autonomous operations for safe, already-wired AI workflows.
-- The public setlist agent remains admin-gated; this internal worker is not executable by client roles.
...
begin
 ...
 if v_song_count=0 then v_risks:=array_append(v_risks,'missing_setlist'); end if;
 if v_repeat_count>0 then v_risks:=array_append(v_risks,'duplicate_song_within_setlist'); end if;
 if v_recent_repeat_count>0 then v_risks:=array_append(v_risks,'recent_song_repetition'); end if;
...
revoke execute on function public.os_run_setlist_operations_agent_internal(uuid) from public,anon,authenticated;

create or replace function public.os_run_scheduled_ai_operations()
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
declare s record; result jsonb; results jsonb:='[]'::jsonb; processed int:=0; skipped int:=0; errors int:=0; v_agent_id uuid;
begin
 select id into v_agent_id from os_agents where key='setlist_agent' and enabled limit 1;
 if v_agent_id is null then return jsonb_build_object('ok',false,'reason','setlist_agent_disabled'); end if;
 for s in select id from music_services where service_date between current_date and current_date+7 order by service_date limit 25 loop
  begin
   if exists(select 1 from os_agent_runs where agent_id=v_agent_id and input_context->>'service_id'=s.id::text and started_at>now()-interval '30 minutes') then skipped:=skipped+1; continue; end if;
   result:=public.os_run_setlist_operations_agent_internal(s.id);
   results:=results||jsonb_build_array(result); processed:=processed+1;
  exception when others then errors:=errors+1; results:=results||jsonb_build_array(jsonb_build_object('ok',false,'service_id',s.id,'error',sqlerrm)); end;
 end loop;
 return jsonb_build_object('ok',errors=0,'processed',processed,'skipped',skipped,'errors',errors,'results',results,'ran_at',now());
end $$;

revoke execute on function public.os_run_scheduled_ai_operations() from public,anon,authenticated;
select cron.schedule('red-point-os-v18-setlist-agent','*/5 * * * *','select public.os_run_scheduled_ai_operations();') where not exists (select 1 from cron.job where jobname='red-point-os-v18-setlist-agent');
